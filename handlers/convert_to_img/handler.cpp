#include "handler.h"
#include <mupdf/fitz.h>
#include <iostream>

int PdfConvertHandler(const PdfConvertDto& dto) {
    std::cout << "\n=== PDF Convert: PDF Conversion ===\n";
    std::cout << "Input file:  " << dto.inputFile << "\n";
    std::cout << "Page number: " << dto.pageNumber << "\n";
    std::cout << "DPI:         " << dto.dpi << "\n";
    std::cout << "Output file: " << dto.outputFile << "\n";
    std::cout << "====================================\n";
    
    // Пример работы с MuPDF
    fz_context* ctx = fz_new_context(NULL, NULL, FZ_STORE_UNLIMITED);
    if (!ctx) {
        std::cerr << "Failed to create MuPDF context\n";
        return 1;
    }

    fz_register_document_handlers(ctx);

    fz_document* doc = fz_open_document(ctx, dto.inputFile.c_str());
    if (!doc) {
        std::cerr << "Failed to open document: " << dto.inputFile << "\n";
        fz_drop_context(ctx);
        return 1;
    }

    fz_pixmap* pix = NULL;
    int result = 0;

    fz_try(ctx) {
        fz_page* page = fz_load_page(ctx, doc, dto.pageNumber);
        fz_matrix transform = fz_scale(dto.dpi / 72.0f, dto.dpi / 72.0f); // 300 DPI
        pix = fz_new_pixmap_from_page(ctx, page, transform, fz_device_rgb(ctx), 0);
        fz_save_pixmap_as_jpeg(ctx, pix, dto.outputFile.c_str(), 90);
        fz_drop_page(ctx, page);
        std::cout << "Success!" << std::endl;
    }
    fz_catch(ctx) {
        std::cerr << "Error!" << std::endl;
        result = 1;
    }

    if (pix) fz_drop_pixmap(ctx, pix);
    fz_drop_document(ctx, doc);
    fz_drop_context(ctx);
    
    std::cout << "====================================\n";
    return result;
}